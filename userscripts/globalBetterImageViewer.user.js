...
(function () {
    ...
    class ImageViewer {
        ...
        constructor(img, canClose) {
            ...
            /**
             * Is the user rotating the image?
             * @type {boolean}
             */
        ...
        _updatePositionByKeyboard() {
            ...
            if (this.isKeyDownDown) {
                ay--;
            }

            if (this.isKeyLeftDown) {
                ax++;
            }

            if (this.isKeyRightDown) {
                ax--;
            }
        ...
        _onMouseMove(e) {
            ...
            if (this.isDragging) {
                ...
                const oldDx = this.cursorX - cx;
                const oldDy = this.cursorY - cy;
                const newDx = newX - cx;
                const newDy = newY - cy;

                const oldAng = Math.atan2(oldDy, oldDx);
                const newAng = Math.atan2(newDy, newDx);

                this._rotate(newAng - oldAng);
            }
         ...
         */
        _onBlur() {
            // isRotating is controlled by the keyboard
            this.isRotating = false;
        }
